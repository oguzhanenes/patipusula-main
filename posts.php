...
<head>
    ...
    <title>PatiPusula | İlanlar</title>
...
<body data-spy="scroll" data-target=".navbar-fixed-top">
    ...
    <section id="content">
        <div class="urgent-posts">
            <h2 style="font-family: 'Space Grotesk', sans-serif;">Acil Talepler</h2>
            <div class="urgent-post-holder">
                <?php
                    $sql = "SELECT * FROM table_animal_information JOIN table_photos_and_videos ON
                    table_animal_information.photo_id = table_photos_and_videos.photo_id JOIN 
                    table_location_information ON table_animal_information.location_id = table_location_information.location_id WHERE status_level = 'Acil'";
        ...
        <div class="other-posts">
            <h2 style="font-family: 'Space Grotesk', sans-serif;">Tüm Talepler</h2>
            <div class="other-post-holder">
                <?php
                    $sql = "SELECT * FROM table_animal_information JOIN table_photos_and_videos ON
                    table_animal_information.photo_id = table_photos_and_videos.photo_id JOIN 
                    table_location_information ON table_animal_information.location_id = table_location_information.location_id WHERE status_level = 'Normal-İlan'";
                    $result = $connect -> query($sql);
                    if($result -> num_rows > 0) {
                        while( $row = $result -> fetch_assoc() ) {
                            echo "<div class='post-card' data-id=" . $row["animal_id"] . ">";
